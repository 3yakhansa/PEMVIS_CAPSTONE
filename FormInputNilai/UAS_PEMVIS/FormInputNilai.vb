...
Public Class FormInputNilai
    ...
    Private Sub LoadMahasiswa()
        Try
            Using conn = GetConnection()
                ...
                Dim cmd As New MySqlCommand("SELECT nim, namaMahasiswa FROM mahasiswa ORDER BY nim", conn)
    ...
    Private Sub cmbNIM_SelectedIndexChanged(sender As Object, e As EventArgs) Handles cmbNIM.SelectedIndexChanged
        Try
            ...
            Using conn = GetConnection()
                conn.Open()
                Dim cmd As New MySqlCommand("SELECT namaMahasiswa FROM mahasiswa WHERE nim=@nim", conn)
    ...
    Private Function ValidasiNilai() As Boolean
        ...
        For Each txt In fields
            ...
            If String.IsNullOrWhiteSpace(txt.Text) Then
                MessageBox.Show("Semua nilai harus diisi!", "Validasi Data", MessageBoxButtons.OK, MessageBoxIcon.Warning)
                txt.Focus()
                Return False
            End If
            ...
            If Not Double.TryParse(txt.Text, nilai) Then
                MessageBox.Show("Nilai harus berupa angka!", "Validasi Data", MessageBoxButtons.OK, MessageBoxIcon.Warning)
                txt.Focus()
                Return False
            End If

            If nilai < 0 Or nilai > 100 Then
                MessageBox.Show("Nilai harus 0 - 100!", "Validasi Data", MessageBoxButtons.OK, MessageBoxIcon.Warning)
                txt.Focus()
                Return False
            End If
    ...
    Private Function DataSudahAda(nim As String, kodeMK As String, semester As String) As Boolean
        Try
            Using conn = GetConnection()
                ...
                Dim cmd As New MySqlCommand("SELECT COUNT(*) FROM nilai WHERE nim=@nim AND kodeMK=@kodeMK AND semester=@semester", conn)
    ...
    Private Sub btnSimpan_Click(sender As Object, e As EventArgs) Handles btnSimpan.Click
        ...
        Try
            If cmbNIM.SelectedValue Is Nothing OrElse cmbMataKuliah.SelectedValue Is Nothing OrElse cmbSemester.Text = "" Then
                MessageBox.Show("Lengkapi data!", "Validasi Data", MessageBoxButtons.OK, MessageBoxIcon.Warning)
                Exit Sub
            End If

            If Not ValidasiNilai() Then Exit Sub

            If DataSudahAda(cmbNIM.SelectedValue.ToString(), cmbMataKuliah.SelectedValue.ToString(), cmbSemester.Text) Then
                MessageBox.Show("Data sudah ada!", "Duplikat Data", MessageBoxButtons.OK, MessageBoxIcon.Information)
                Exit Sub
            End If
    ...
    Private Sub btnHapus_Click(sender As Object, e As EventArgs) Handles btnHapus.Click
        ...
        If MessageBox.Show("Hapus data?", "Konfirmasi", MessageBoxButtons.YesNo, MessageBoxIcon.Question) = DialogResult.No Then Exit Sub

        Try
            Using conn = GetConnection()
                conn.Open()
                Dim cmd As New MySqlCommand("DELETE FROM nilai WHERE idNilai=@id", conn)
                cmd.Parameters.AddWithValue("@id", idNilaiEdit)
                cmd.ExecuteNonQuery()
            End Using
        ...
        Try
            Using conn = GetConnection()
                ...
                Dim query As String =
                    ...
                    JOIN mahasiswa m ON n.nim=m.nim
                    JOIN matakuliah mk ON n.kodeMK=mk.kodeMK
                    WHERE n.nim LIKE @c OR m.namaMahasiswa LIKE @c OR n.kodeMK LIKE @c OR mk.namaMK LIKE @c OR n.semester LIKE @c
                    ORDER BY n.idNilai"
